...
contract('Issuers', (accounts) => {
    ...
    it('Removing a non-existing Issuer', async () => {
        ...
        const status = await instance.getIssuerStatus(issuer1address);
        assert.ok(status.eqn(IssuerStatus.None), 'Issuer staus should be 0 (non-existing)');
    });

    it('Adding an Issuer', async () => {
        await instance.addIssuer(issuer1address, { from: ownerAddress });
        const status = await instance.getIssuerStatus(issuer1address);
        assert.ok(status.eqn(IssuerStatus.Active), 'Issuer staus should be 1 (active)');
    });

    it('Removing an existing Issuer', async () => {
        await instance.removeIssuer(issuer1address, { from: ownerAddress });
        const status = await instance.getIssuerStatus(issuer1address);
        assert.ok(status.eqn(IssuerStatus.Inactive), 'Issuer staus should be 2 (inactive)');
    });

    it('Re-adding an existing inactive Issuer', async () => {
        // First, make sure the user is inactive
        assert.ok((await instance.getIssuerStatus(issuer1address)).eqn(IssuerStatus.Inactive), 'Issuer staus should be 2 (inactive)');
        await instance.addIssuer(issuer1address, { from: ownerAddress });
        const status = await instance.getIssuerStatus(issuer1address);
        assert.ok(status.eqn(IssuerStatus.Active), 'Issuer staus should be 1 (active)');
    });
